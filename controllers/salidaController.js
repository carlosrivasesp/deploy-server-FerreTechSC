...
exports.registrarSalida = async (req, res) => {
  try {
    ...
    let fechaSalidaLocal = null;

    if (fechaSalida) {
      const [y, m, d] = fechaSalida.split("-");
      fechaSalidaLocal = new Date(y, m - 1, d);
    }
    ...
    if (operacion.tipoOperacion !== 1) {
      return res
        .status(400)
        .json({ message: "Solo se pueden registrar salidas para pedidos" });
    }
    ...
    if (!estadosPermitidos.includes(operacion.estado)) {
      return res.status(400).json({
        message:
          "Solo se puede registrar salida para pedidos con estado Enviado o Entregado",
      });
    }
    ...
    const totalPrevio = totalSalidasPrevias.length
      ? totalSalidasPrevias[0].total
      : 0;
    const cantidadPedido = operacion.detalles.reduce(
    ...
    if (totalPrevio >= cantidadPedido) {
      return res
        .status(400)
        .json({ message: "Este pedido ya fue completamente despachado" });
    }
    ...
    const nuevaSalida = new Salida({
      ...
      pedidoId,
      cantidadTotal,
      fechaSalida: fechaSalidaLocal, // 🔥 FIX DEFINITIVO
      detalles,
    });

    const salidaGuardada = await nuevaSalida.save();

    const detallesConProductos = await Salida.findById(
      salidaGuardada._id
    ).populate({
      path: "detalles.detalleId",
      populate: { path: "producto", model: "Producto" },
    ...
    await Operacion.findByIdAndUpdate(pedidoId, {
      $push: { salidas: salidaGuardada._id },
    });
