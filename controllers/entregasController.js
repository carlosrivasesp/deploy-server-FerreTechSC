...
exports.getEntregas=async(req,res)=>{
    try{
        const entregas = await Entregas.find()
            .populate({
                path: 'ventaId',               
                populate: [
                  {path:'detalles'},
...
exports.getEntrega=async(req,res)=>{
    try{
        let entrega=await Entregas.findById(req.params.id)
        .populate({
                path: 'ventaId',               
                populate: [
                  {path:'detalles'},
...
exports.updateEntrega = async (req, res) => {
  try {
    const entrega = await Entregas.findById(req.params.id);
    ...
    if (estadoActual === "Pendiente" && nuevoEstado === "En proceso") {
      ...
      //Aquí agregas los campos nuevos del frontend
      entrega.direccion = req.body.direccion || entrega.direccion;
      entrega.fechaInicio = req.body.fechaInicio || entrega.fechaInicio;
      entrega.fechaFin = req.body.fechaFin || entrega.fechaFin;
...
const exportarEntregas = async (entregas, res, nombreArchivo) => {
    try {
        if (entregas.length === 0) {
            return res.status(400).json({ message: 'No hay entregas para exportar.' });
        }
...
exports.entregasRealizadas = async (req, res) => {
  try {
    const entregas = await Entregas.find({estado:"Finalizado"})
      .populate({
        path: 'ventaId',
        populate: {
          path: 'lugar',
...
exports.entregasPendientes = async (req, res) => {
  try {
    const entregas = await Entregas.find({estado:"Pendiente"})
      .populate({
        path: 'ventaId',
        populate: {
          path: 'lugar',
...
exports.entregasProgramadas = async (req, res) => {
  try {
    const entregas = await Entregas.find({estado:"En proceso"})
      .populate({
        path: 'ventaId',
        populate: {
          path: 'lugar',
